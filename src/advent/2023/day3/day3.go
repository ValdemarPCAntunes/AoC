...
func part1(schematic []string) int {
	sum := 0
	// lineLimit := len(schematic)
	for i, line := range schematic {
		for j, letter := range line {
			if unicode.IsSymbol(letter) {
				top := sumLine(schematic, i-1, j)
				middle := sumLine(schematic, i, j)
				bottom := sumLine(schematic, i+1, j)
...
func isOutOfBounds(schematic []string, i int, j int, lineLimit int, colLimit int) bool {
	return i < 0 || i >= len(schematic) || j < 0 || j >= len(schematic[0])
}

func sumLine(schematic []string, line int, start int) int {
	// if unicode.IsDigit(schematic[line][start]) {

	// }
	return 0
}

func sumLeft(schematic []string, line int, start int) int {

	return 0
}

func sumRight(schematic []string, line int, start int) int {
