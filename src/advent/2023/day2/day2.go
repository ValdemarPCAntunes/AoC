...
const c_red, c_green, c_blue = "red", "green", "blue"
...
func part1(game string, config map[string]int) bool {
	startIdx := strings.Index(game, ": ")

	gameSets := strings.Split(game[startIdx+2:], "; ")
...
func part2(game string) int {
	...
	power := 1
	// fmt.Printf("Set found {")
	for _, val := range colors {
		power *= val
		// fmt.Printf("%s: %d ", c, val)
	}
	// fmt.Printf("}\nPower: %d", power)
	return power
}
...
func main() {
	...
	part1Config := map[string]int{"red": 12, "green": 13, "blue": 14}
	powerOfSets := 0
	for i, game := range strings.Split(games, "\n") {
		fmt.Println("\nReading " + game)
		if part1(strings.TrimRight(game, "\r"), part1Config) {
			totalGames += 1
			sumOfGameIds += i + 1
			possibleGames += strconv.Itoa(i+1) + ", "
		}
		powerOfSets += part2(strings.TrimRight(game, "\r"))
	}

	printResultP1(totalGames, sumOfGameIds, part1Config, possibleGames)
	printResultP2(powerOfSets)
}

func printResultP1(games int, sumOfIds int, config map[string]int, possibleGames string) {
	fmt.Printf("\nFor config %d red, %d green, %d blue cubes\n", config[c_red], config[c_green], config[c_blue])
	fmt.Printf("\nSum of game ids: %d \n", sumOfIds)
	fmt.Printf("\nThere are %d possible games\n", games)
	fmt.Printf("\nPossible games are [%#v]\n", strings.Trim(possibleGames, ", "))
}

func printResultP2(power int) {
	fmt.Printf("\nSum of the power of the sets: %d\n", power)
}
